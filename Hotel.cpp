...
namespace gestion {
	...
	int Hotel::testAndAddClient() {
		int id = -1;
		//On demande le nom du client ayant fait la réservation
	...
	void Hotel::setDatesAndRoom(Reservation& resa) {
		...
		//On demande ensuite la remise sur le séjour
			...
			if (type == it->getType()) {
				...
				while (it1 != _reservationsliste.end()) {
					if (it->getIdChambre() == it1->getIdChambre()) { // test l'ID de la chambre et la période de séjour (relation reservation/chambre)
						...
						switch (index) { // on test la disponibilitée de plusieurs réservations sur des périodes différentes
						case 0:
