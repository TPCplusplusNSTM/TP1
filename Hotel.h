...
namespace gestion {
	...
	public:
		...
		int testAndAddClient(); // fonction utilisée dans createReservation : vérifie si le client est dans l'hotel et retourne son ID / sinon crée un nouveau client avec un ID valide
		...
		void setDatesAndRoom(Reservation& resa); // permet à l'utilisateur de définir les dates de son séjour et le type de chambre
		...
		int newIdClient(); // crée un identifiant utilisable
		int newIdReservation();

		int checkTypeDispo(genre type, date::Date ddebut, date::Date dfin) const; // permet de retourner l'index d'une chambre disponible, en fonction du genre et de la période disponible.
